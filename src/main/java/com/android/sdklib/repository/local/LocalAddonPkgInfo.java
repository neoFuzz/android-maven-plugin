...
public class LocalAddonPkgInfo extends LocalPlatformPkgInfo {
    ...
    protected IAndroidTarget createAndroidTarget() {
        ...
        try {
            ...
            Map<String, File> skinsMap = new TreeMap<>();
            ...
            List<File> skins = new ArrayList<>(skinsMap.values());
    ...
    /**
     * Converts a string representation of a hexadecimal ID into an int.
     *
    ...
    private ISystemImage[] getAddonSystemImages(IFileOp fileOp) {
        ...
        for (File file : files) {
            ...
            } else if (!hasImgFiles && fileOp.isFile(file) && file.getName().endsWith(".img")) {                  //$NON-NLS-1$
