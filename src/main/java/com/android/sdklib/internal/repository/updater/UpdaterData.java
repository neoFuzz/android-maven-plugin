...
public class UpdaterData implements IUpdaterData {
    ...
    protected void initSdk() {
        ...
        try {
            mAvdManager = null;
            mAvdManager = AvdManager.getInstance(
                    AndroidSdkHandler.getInstance(mSdkManager.getLocalSdk().getLocation()), mSdkLog);
    ...
    protected List<Archive> installArchives(final List<ArchiveInfo> archives, final int flags) {
        ...
        final List<Archive> newlyInstalledArchives = new ArrayList<>();
        ...
        mTaskFactory.start("Installing Archives", monitor -> {
            ...
            HashSet<Archive> installedArchives = new HashSet<>();
            ...
            for (ArchiveInfo ai : archives) {
                ...
                try {
                    ...
                    if (msg != null) {
                        ...
                        assert archive.getParentPackage() != null;
                        msg = String.format("Unexpected Error installing '%1$s'%n%2$s",
                                archive.getParentPackage().getShortDescription(), baos);
                    }
    ...
    public List<Archive> updateOrInstallAll_NoGUI(
        ...
        if (pkgFilter != null && !pkgFilter.isEmpty() && archives != null && !archives.isEmpty()) {
            ...
            HashMap<String, Package> installIdMap = new HashMap<>();
            for (ArchiveInfo ai : archives) {
                Archive a = ai.getNewArchive();
                if (a != null) {
                    Package p = a.getParentPackage();
            ...
            Set<String> userFilteredInstallIds = new HashSet<>();
    ...
    boolean acceptLicense(
        ...
        TreeSet<String> acceptedLids = new TreeSet<>();
        ...
        TreeSet<String> rejectedLids = new TreeSet<>();
        TreeMap<String, License> lidToAccept = new TreeMap<>();
        TreeMap<String, List<String>> lidPkgNames = new TreeMap<>();
    ...
    public void refreshSources(final boolean forceFetching) {
        ...
        mTaskFactory.start("Refresh Sources", monitor -> {
    ...
    private static class InstallOrderComparator implements Comparator<ArchiveInfo> {
        ...
        private final Map<ArchiveInfo, Integer> mOrders = new HashMap<>();
