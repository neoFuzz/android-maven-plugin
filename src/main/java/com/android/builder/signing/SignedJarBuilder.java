...
public class SignedJarBuilder {
    ...
    private static boolean processMetaInf(@Nonnull String name) {
        if (name.startsWith("META-INF/")) {
            ...
            return index == -1 && (subName.endsWith(".SF") || name.endsWith(".RSA") || name.endsWith(".DSA"));
     ...
     * @throws IOException if an I/O error occurs.
     */
    @SuppressWarnings("unused") // used by external libraries at runtime
    ...
    public void writeZip(InputStream input, IZipEntryFilter filter, ZipEntryExtractor extractor)
        ...
        try (ZipInputStream zis = new ZipInputStream(input)) {
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                if (processMetaInf(name)) continue;
    ...
    private void writeEntry(@NonNull InputStream input, JarEntry entry) throws IOException {
        ...
        if (mManifest != null) {
            ...
            attr.putValue(DIGEST_ATTR,
                    new String(Base64.encode(mMessageDigest.digest()), StandardCharsets.US_ASCII));
        }
    }
    ...
    private void writeSignatureFile(OutputStream out)
        ...
        print.flush();
        main.putValue(DIGEST_MANIFEST_ATTR, new String(Base64.encode(md.digest()), StandardCharsets.US_ASCII));
        ...
        for (Map.Entry<String, Attributes> entry : entries.entrySet()) {
            ...
            sfAttr.putValue(DIGEST_ATTR, new String(Base64.encode(md.digest()), StandardCharsets.US_ASCII));
