...
public class AarMojo extends AbstractAndroidMojo {
    ...
    @Parameter
    @PullParameter
    private final String[] classesJarIncludes = new String[]{"**/*"};
    ...
    @Parameter
    @PullParameter
    private final String[] classesJarExcludes = new String[]{"**/R.class", "**/R$*.class"};
     ...
     */
    @Inject
    protected AarMojo(ArtifactResolver artifactResolver, ArtifactHandler artHandler, MavenProjectHelper projectHelper, DependencyGraphBuilder dependencyGraphBuilder) {
    ...
    private void addNativeLibraries(final ZipArchiver zipArchiver) throws MojoExecutionException {
        ...
        // TODO: inspect and write a test
        // Check for compiled native libraries from this build
        // First check attached artifacts
        for (Artifact attachedArtifact : project.getAttachedArtifacts()) {
            if ("so".equals(attachedArtifact.getType()) && attachedArtifact.getFile() != null) {
                String architecture = attachedArtifact.getClassifier() != null ? attachedArtifact.getClassifier() : "armeabi";
                String dest = NATIVE_LIBRARIES_FOLDER + "/" + architecture + "/" + attachedArtifact.getFile().getName();
                zipArchiver.addFile(attachedArtifact.getFile(), dest);
                getLog().debug("Added attached native library: " + attachedArtifact.getFile().getName());
            }
        }
        
        // Check target directory for compiled .so files (from NDK build)
        File[] targetSoFiles = targetDirectory.listFiles((dir, name) -> name.endsWith(".so"));
        if (targetSoFiles != null) {
            for (File soFile : targetSoFiles) {
                String architecture = ndkArchitecture != null ? ndkArchitecture : "armeabi";
                String dest = NATIVE_LIBRARIES_FOLDER + "/" + architecture + "/" + soFile.getName();
                zipArchiver.addFile(soFile, dest);
                getLog().debug("Added compiled native library from target: " + soFile.getName());
            }
        }
    }
