...
public final class ClasspathModifierLifecycleParticipant extends AbstractMavenLifecycleParticipant {
    ...
    @Override
    public void afterProjectsRead(@NonNull MavenSession session) throws MavenExecutionException {
        log.debug("");
    ...
    private void handleArtifactsForClasspath(MavenProject project, @NonNull Set<Artifact> artifacts,
        ...
        for (Artifact artifact : artifacts) {
            ...
            if (type.equals(AndroidExtension.AAR)) {
                ...
                // It will be replaced with the real classes.jar by GenerateSourcesMojo.
                ...
                // The only time that an APK will likely be a dependency is when this an APK test project.
     ...
     */
    private void addLibsJarsToClassPath(@NonNull UnpackedLibHelper helper, MavenProject project, Artifact artifact) {
        try {
            final File unpackLibFolder = helper.getUnpackedLibFolder(artifact);
            final File artifactFile = helper.getArtifactToFile(artifact);

            // Use try-with-resources to automatically close ZipFile when done
     ...
     */
    private void addClassesToClasspath(@NonNull UnpackedLibHelper helper, MavenProject project, Artifact artifact)
            throws MavenExecutionException {
        ...
        if (!classesJar.exists()) {
            ...
            try (ZipOutputStream zipOutputStream = new ZipOutputStream(new FileOutputStream(classesJar))) {
        ...
        // any existing dependencies with the same information.
    ...
    @NonNull
    private Dependency createSystemScopeDependency(@NonNull Artifact artifact, File location, String suffix) {
    ...
    @Nullable
    private Dependency findProvidedDependencies(Dependency dexDependency, @NonNull MavenProject project) {
        for (Dependency dependency : project.getDependencies()) {
            if (dependency.getScope().equals(Artifact.SCOPE_PROVIDED) &&
                    dependency.getArtifactId().equals(dexDependency.getArtifactId())
