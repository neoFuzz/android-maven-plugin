...
import com.github.cardforge.maven.plugins.android.common.*;
import com.github.cardforge.maven.plugins.android.config.ConfigPojo;
import com.github.cardforge.maven.plugins.android.configuration.Jack;
import com.github.cardforge.maven.plugins.android.configuration.Ndk;
import com.github.cardforge.maven.plugins.android.configuration.Sdk;
import com.github.cardforge.maven.plugins.android.phase_prebuild.ClasspathModifierLifecycleParticipant;
...
public abstract class AbstractAndroidMojo extends AbstractMojo {
    ...
    /**
     * The supported package types list
     */
    protected static final List<String> SUPPORTED_PACKAGING_TYPES = new ArrayList<>();
    /**
    ...
    /**
     * The android {@code assets} directory.
     */
    ...
    /**
     * File with the extracted Java sources' dependencies
     */
    @Parameter(
    ...
    /**
     * File with the extracted Java resources' dependencies
     */
    @Parameter(
    ...
    /**
     * The combined {@code assets} directory. This will contain both the assets found in "assets" and any assets
     * contained in an {@code apksources}, {@code apklib} or {@code aar} dependencies.
     */
     ...
     * device interaction goals support this so you can deploy the apk to all attached emulators and devices.
     ...
     * each IP address. If you have external device, you should call this connect goal before any other goal :
     * mvn clean android:connect install.</p>
     * <p>The Maven plugin will automatically add all these IP addresses into the devices parameter.
     ...
     * defined in the AndroidManifest.xml. These files are then automatically used in the proguard mojo execution,
     * if enabled.
     */
    ...
    /**
     * The ADB connection timeout value to use.
     */
     ...
     * <p>Corresponds to {@code Sdk#path}.</p>
     */
     ...
     * <code>&lt;sdk&gt;</code> configuration tag.</p>
     * <p>Corresponds to {@code Sdk#platform}.</p>
     */
     ...
     * <p>Corresponds to {@code Ndk#path}.</p>
     */
    ...
    private Jack jack;
    /**
     * Helper for unpacking libraries.
     */
    private UnpackedLibHelper unpackedLibHelper;
    /**
     * Helper for artifact resolver.
     */
     ...
     */
    @Inject
    protected AbstractAndroidMojo(ArtifactResolver artifactResolver, ArtifactHandler artHandler,
                                  MavenProjectHelper projectHelper, DependencyGraphBuilder dependencyGraphBuilder) {
        this.artifactHandler = artHandler;
    ...
    /**
     * @return a {@code DependencyResolver} which can be used to resolve dependencies. Never {@code null}.
     */
    @NonNull
    ...
    protected Set<Artifact> getRelevantCompileArtifacts() {
        final List<Artifact> allArtifacts = project.getArtifacts().stream()
                .filter(a -> a.getArtifactHandler().isAddedToClasspath())
                .filter(a -> Set.of("compile", "provided", "system").contains(a.getScope()))
                .toList();
    ...
    protected Set<Artifact> getDirectDependencyArtifacts() {
        final Set<Artifact> allArtifacts = project.getArtifacts().stream()
                .filter(artifact -> !artifact.hasClassifier())
     ...
     *
     * @return the Android Debug Bridge
     * @throws MojoExecutionException If there is a problem initializing the Android Debug Bridge.
     */
    public AndroidDebugBridge initAndroidDebugBridge() throws MojoExecutionException {
        synchronized (ADB_LOCK) {
            if (!adbInitialized) {
                DdmPreferences.setTimeOut(getAdbConnectionTimeout());
     ...
     * @throws MojoExecutionException If there is a problem waiting for the initial device list.
     */
    protected void waitForInitialDeviceList(@NonNull final AndroidDebugBridge androidDebugBridge)
     ...
     * @throws MojoFailureException   If there is a problem deploying the apk file.
     */
    protected void deployApk(final File apkFile) throws MojoExecutionException, MojoFailureException {
        ...
        doWithDevices(iD -> {
            String deviceLogLinePrefix = DeviceHelper.getDeviceLogLinePrefix(iD);
            try {
                iD.installPackage(apkFile.getAbsolutePath(), true);
                getLog().info(deviceLogLinePrefix + "Successfully installed " + apkFile.getAbsolutePath());
                getLog().debug(" to " + DeviceHelper.getDescriptiveName(iD));
    ...
    /**
     * @throws MojoExecutionException If there is a problem with the undeploy of the apk file.
     * @throws MojoFailureException   If there is a problem with the undeploy of the apk file.
     */
    protected void deployDependencies() throws MojoExecutionException, MojoFailureException {
        Set<Artifact> directDependentArtifacts = project.getArtifacts().stream()
                .filter(artifact -> artifact.getDependencyTrail().size() == 2) // Size 2 means direct dependency (first is the project itself)
                .collect(Collectors.toSet());
    ...
    public void doWithDevices(final DeviceCallback deviceCallback)
        ...
        if (shouldRunOnAllDevices) {
            ...
            getLog().info("android.devices parameter set to " + getDevices());
        }

        ArrayList<ActionRunnable> doThreads = getThreads(deviceCallback, threadCount, deviceList, shouldRunOnAllDevices);
        throwAnyDoThreadErrors(doThreads);

        if (!shouldRunOnAllDevices && doThreads.isEmpty()) {
            throw new MojoExecutionException("No device found for android.device=" + getDevices());
        }
    }

    @NonNull
    private ArrayList<ActionRunnable> getThreads(DeviceCallback deviceCallback, int threadCount,
                                                 @NonNull List<IDevice> deviceList, boolean shouldRunOnAllDevices) {
        ArrayList<ActionRunnable> doThreads = new ArrayList<>();
        ExecutorService executor = Executors.newFixedThreadPool(threadCount);
        for (final IDevice idevice : deviceList) {
            if (shouldRunOnAllDevices) {
            ...
            if (shouldRunOnAllDevices || shouldDoWithThisDevice(idevice)) {
                ActionRunnable deviceDoThread = new ActionRunnable() {
        ...
        while (!executor.isTerminated()) { // NOSONAR
            // waiting for threads finish
        }
        return doThreads;
    }

    private void throwAnyDoThreadErrors(@NonNull ArrayList<ActionRunnable> doThreads) throws MojoExecutionException,
            MojoFailureException {
        for (ActionRunnable deviceDoThread : doThreads) {
     ...
     */
    private boolean shouldDoWithThisDevice(IDevice iDevice) {

        for (String d : getDevices()) {
            // use specified device or all emulators or all devices
            if ("emulator".equals(d) && iDevice.isEmulator()) {
                return true;
            }

            if ("usb".equals(d) && !iDevice.isEmulator()) {
                return true;
            }

            if (iDevice.isEmulator() && (d.equalsIgnoreCase(iDevice.getAvdName()) || d
                    .equalsIgnoreCase(iDevice.getSerialNumber()))) {
                return true;
            }

            if (!iDevice.isEmulator() && d.equals(iDevice.getSerialNumber())) {
                return true;
            }
        }
    ...
    /**
     * Undeploy an apk from a connected emulator or usb device. Also deletes the application's data and cache
     * directories on the device.
     *
     * @param apkFile the file to undeploy
     * @return <code>true</code> if successfully undeploy, <code>false</code> otherwise.
     * @throws MojoExecutionException if there is a problem undeploying the apk file.
     * @throws MojoFailureException   if there is a problem undeploying the apk file.
     */
    ...
    /**
     * Undeploy an apk, specified by package name, from a connected emulator
     * or usb device. Also deletes the application's data and cache
     * directories on the device.
     *
     * @param packageName the package name to undeploy.
     * @return <code>true</code> if successfully undeploy, <code>false</code> otherwise.
     * @throws MojoExecutionException if there is a problem un-deploying the apk file.
     * @throws MojoFailureException   if there is a problem un-deploying the apk file.
     */
    protected boolean undeployApk(final String packageName) throws MojoExecutionException, MojoFailureException {
        ...
        doWithDevices(d -> {
            String deviceLogLinePrefix = DeviceHelper.getDeviceLogLinePrefix(d);
            try {
                d.uninstallPackage(packageName);
                getLog().info(deviceLogLinePrefix + "Successfully uninstalled " + packageName);
                getLog().debug(" from " + DeviceHelper.getDescriptiveName(d));
                result.set(true);
            } catch (InstallException e) {
                result.set(false);
    ...
    public String extractPackageNameFromApk(@NonNull File apkFile) throws MojoExecutionException {
        ...
        getLog().info(getAndroidSdk().getAaptPath() + " " + commandBuilder);
        try {
            ...
            if ((standardError != null) && (!standardError.trim().isEmpty())) {
     ...
     *
     * @param aaptDumpXmlTree output from <code>aapt dump xmltree &lt;apkFile&gt; AndroidManifest.xml</code>
     * @return the package name from inside the apkFile.
     */
    public String extractPackageNameFromAndroidManifestXmlTree(String aaptDumpXmlTree) {
        String packageName;
        try (Scanner scanner = new Scanner(aaptDumpXmlTree)) {
            ...
            scanner.findWithinHorizon("  A: package=\"", 0); // NOSONAR : must be important to have the double space
     ...
     * @return the package name from the AndroidManifest.xml file.
     */
     ...
     * @throws MojoExecutionException if there is a problem reading the AndroidManifest.xml file.
     */
    public String extractInstrumentationRunnerFromAndroidManifest(@NonNull File manifestFile)
            throws MojoExecutionException {
        final URL xmlURL;
        try {
    ...
    /**
     * @return boolean if the file is a test
    ...
    /**
     * @return the Jack toolchain to use, or <code>null</code> if Jack is not being used.
     */
    protected Jack getJack() {
        return Objects.requireNonNullElseGet(jack, () -> new Jack(super.getPluginContext()));
    }

    /**
     * @return the Android SDK path to use, or throws an exception if it cannot be found.
     * @throws MojoExecutionException if the Android SDK path could not be found.
     */
    ...
    /**
     * @return Folder where the unpacked libraries are located.
     */
    ...
    /**
     * @param artifact Android dependency that is being referenced.
     * @return Jar file where the unpacked classes are located.
     */
    @NonNull
    ...
    /**
     * @param artifact Android dependency that is being referenced.
     * @return Folder where the unpacked source code is located.
     */
    @NonNull
    ...
    /**
     * @param artifact Android dependency that is being referenced.
     * @return Folder where the unpacked resources are located.
     */
    @NonNull
    ...
    /**
     * @param artifact Android dependency that is being referenced.
     * @return Folder where the unpacked assets are located.
     */
    @NonNull
    ...
    /**
     * @return the Android NDK path to use, or throws an exception if it cannot be found.
     * @throws MojoExecutionException if the Android NDK path could not be found.
     */
    ...
    /**
     * @return The set of devices to use for the emulator.
     */
    @NonNull
     ...
     *
     * @return The timeout value for an adb connection in milliseconds.
     */
    ...
    /**
     * @param adbConnectionTimeout The timeout value for an adb connection in milliseconds.
     */
     ...
     * <p>
     * The target folder doesn't exist it will be created.
     * </p>
     *
     ...
     * @throws MojoExecutionException if the files cannot be copied.
     */
    private void copyFolder(@NonNull File sourceFolder, File targetFolder, FileFilter filter)
            throws MojoExecutionException {
    ...
    /**
     * @return the unpacked lib helper
     */
    ...
    /**
     * @return the artifact resolver helper
     */
    ...
    /**
     * Helper class for running threads.
     */
    private abstract static class ActionRunnable implements Runnable {
        private MojoFailureException failure;
        private MojoExecutionException execution;

        /**
         * Runs the thread and handles any exceptions.
         */
        @Override
         ...
         * @throws MojoExecutionException if an execution exception occurs
         */
